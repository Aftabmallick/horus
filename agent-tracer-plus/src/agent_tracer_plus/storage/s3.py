...
class S3Backend(StorageBackend):
    ...
    async def _ensure_initialized(self):
        try:
            ...
            raise ImportError("aioboto3 is required for S3 backend. Run `pip install agent-tracer-plus[s3]`")
    ...
    async def _periodic_flush(self):
        while True:
            try:
                ...
                logger.error(f"Error in S3 periodic flush: {e}")
    ...
    async def get_trace(self, trace_id: str) -> Optional[Trace]:
        raise NotImplementedError("Direct reading of traces from S3 is not supported. Use a database backend for querying.")

    async def get_spans(self, trace_id: str) -> List[Span]:
        raise NotImplementedError("Direct reading of spans from S3 is not supported.")

    async def query_traces(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        raise NotImplementedError("Querying traces from S3 is not supported.")

    async def delete_traces(self, before: datetime) -> int:
        raise NotImplementedError("S3 Lifecycle rules should be used for trace deletion.")
    ...
    async def flush(self) -> None:
        ...
        if spans_to_upload:
            try:
                ...
                logger.debug(f"Flushed {len(spans_to_upload)} spans to S3 at {key}")
            except Exception as e:
                logger.error(f"Failed to flush spans to S3: {e}")
